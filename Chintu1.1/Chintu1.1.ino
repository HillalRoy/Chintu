...
#define trig_pin1 A3 //analog input 1
#define echo_pin1 A4 //analog input 2


#define maximum_distance 200
boolean goesForward = false;
int distance = 100;
int heightclearance=100;
...
void loop() {
  ...
  if (distance <= 40 || heightclearance<=40) {
    Serial.println("Obstacle ahead or no height clearance");
    digitalWrite(buzzer, HIGH);
    moveStop();
    delay(300);
    moveBackward();
  ...
  Serial.println(heightclearance);
  distance = readPing();
  heightclearance=readHeight();
  delay(200);
}
